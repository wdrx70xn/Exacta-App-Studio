...
import { AI_STREAMING_ERROR_MESSAGE_PREFIX, getContextOverflowMessage } from "@/shared/texts";
        ...
        const simpleStreamText = async ({
          ...
          const streamResult = streamText({
            ...
            onError: (error: any) => {
              ...
              // Check for context overflow and provide a friendly message
              const contextOverflowMsg = getContextOverflowMessage(errorMessage);
              if (contextOverflowMsg) {
                logger.warn("Context window overflow detected, showing friendly message to user");
                event.sender.send("chat:response:error", {
                  chatId: req.chatId,
                  error: contextOverflowMsg,
                });
